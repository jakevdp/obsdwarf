...
In section  \ref{sec:halpha} we briefly summarize each of the observable
...
compiled a catalog of clusters with known redshifts using the 
Abell \citep{abe89} and ROSAT \citep{ebe96} bright
clusters. We remove many clusters from the Abell catalog as they lack spectroscopic
...
In this section we summarize the various predictions of modified gravity
which will be explored in later sections.  The first three effects are
expected to appear in the presence of a sufficiently strong external
potential gradient, e.g.\ the gravitational attraction of a neighboring
dwarf galaxy (or the effective repulsion of a void).
The fourth effect has no environmental coupling, but is due to the
internal gravitational dynamics of the 
galaxy itself.  Quantitative estimates of the magnitude of these
observables are given  in the following sections. For more
details on these predictions see \citet{bhuvjake2011}. 

\begin{enumerate}
\item \textbf{Offset between stellar and gaseous components:}
Modified gravity theories predict that in the presence of a suitable
potential gradient, low density, unscreened components like dark matter or
neutral hydrogen clouds (HI) will outpace screened dense stars in
falling along the potential.
This may cause a measurable offset between the
centroid of stellar disk and that of gaseous disk. The size of the offset
depends strongly on the mass and concentration of the dwarf galaxy's halo,
as well as the distance to neighboring galaxies.
Observationally, this effect may be manifest as an offset between the
centroids of the gaseous and stellar disk ({\it see section \ref{sec:h1-opt-offset}-\ref{sec:kinematical}}).
...
galaxy halo, as well as the strength of the external potential gradient. The warp  
...
varies significantly depending on the external fifth force acting on the dwarf galaxy.  
{\it See section \ref{sec:warp} for this test}. 
%\jakesays{Note expected magnitude of this effect.}

\item \textbf{Asymmetry in the stellar rotation curve:}
If a disk galaxy is falling edge-on, or more generally if the external potential gradient is not aligned with the galaxy's axis of
rotation, the offset between stellar and halo centers can perturb the stellar disk
...
can be as high as 10 km/s, and is observable by comparing the
approaching and receding stellar rotation curves.
The asymmetry can be distinguished from standard astrophysical sources, for example by also observing the rotation of the gaseous component:
if the asymmetry of the stellar rotation curve is  due to modified
gravity, the rotation curve of the gas for the same dwarf galaxy should be near
symmetric.  {\it A version of this test is show in section 6.2}. 

\item \textbf{Rotation speeds of Gaseous and Stellar disk:}
The rotation velocity of an unscreened gaseous
disk will enhanced by a factor of
...
the stellar and gaseous rotation will give a limit on the fifth force. The
enhancement in velocity can be $\sim 10$ km/s under reasonable assumptions. 
This effect arises due to the self-gravity of the galaxy. 
{\it See section 6.1 for a discussion of current data and its limitations}. 
...
For $f_{R0} = 1\times10^{-6} (2\times10^{-7})$, this potential is only sufficient
...
by the stellar potential is negligible.  Therefore, we would expect  that the HII region feels the fifth force and 
...
\end{eqnarray}
where  $\mathcal{N}$ is the Gaussian variable and $\sigma_i$ is the (known)
...
rule-out modified gravity theories: a better test would limit the sample to
...
on the current sample leaves too few galaxies to perform a meaningful analysis.
...
\begin{center}
  ...
  \caption{The posterior of the Bayesian model used to distinguish separation
    due to modified gravity and due to other astrophysical effects (see
    discussion of this model in Sec \ref{sec:h1-opt-offset}).  The left
...
small sample size.  Additionally, because red giants are only
...
null hypotheses is accepted only with a probability $\sim$ 0.88.
...
assume each galaxy
will have an acceleration of $\sim 10^{-15}$ km/s. Also, we need to consider
...
This analysis gives $w_{MG}= 0.003_{0.001}^{0.003}$ for
...
type of warp included in the analysis. All - included all galaxies, U - used only U-type warps. 
...
\begin{tabular}{ || l | p{4cm} | p{4cm} | p{4cm} ||}
        \hline
{\bf PROBES} &  {\bf \ha~ asymmetry}    &  {\bf HI-H$\alpha$}   & {\bf                                                                
Kinematical offset}    \\
        \hline
\\
...
\\
   ...
   & $N_U^{-6}=68$, $N_S^{-6}=58$  &  & $N_U^{-6.5}=241$, $N_S^{-6.5}=441$ \\
\\

  &   &  & $N_U^{-6}=635$, $N_S^{-6}=47$ \\
\\
{\bf Result}   & $a_{MG}^{-6.5}=0.024\pm 0.015$   & 
 $v_{HI-H\alpha}= 0.11\pm 1.50 \pm 5$ km/s & $o_{MG}^{-7}=-0.01\pm0.006$\\
   &  $a_{MG}^{-6}=-0.001\pm 0.012$ & 
$\Delta v/v_{HI}= 0.006\pm 0.020 \pm 0.060$  & $o_{MG}^{-6.5}=0.003\pm0.003$ \\
   &   &   & $o_{MG}^{-6}=-0.003\pm0.005$\\
\\
{\bf Conclusions}   & A 10\% asymmetry created by modified gravity can be
ruled out with 5$\sigma$ confidence for $f_{R0}=10^{-6}$ and
$f_{R0}=10^{-6.5}$& \ha{} follows gaseos component &
The intrinsic offset is around 2\% of the optical
radius, both for unscreened and screened galaxies. The resulting modified
gravity offset if less than 1\% always and consistent with zero  \\
\hline
\end{tabular}
...
competetive constraints in the future.
...
Finally, we show that the offset between the partially-screened red giant
...
More detailed studies of these individual cases may shed light
...
Another important caveat is that we use a large, hetrogenous data set to
perform the above tests.  Because these data were gathered without this
type of test in mind, there was little specific attempt to account for
...
\item {\it Offset between HI and optical image:}
  ...
  Using the ALFALFA data, the error in the offset estimate is $\sim$ 2 kpc;
  to get an error of 50 (10) pc we need to stack $\sim 2\times10^{3}$
  ($4\times10^{4}$) galaxies.  Furthermore, the characteristics of the error
...
\item {\it Warping in edge-on galaxies:}
  ...
  These numbers are feasible with the current and ongoing galaxy surveys.
  ...
  this mass profile.
\item {\it Direct measurement of $\Delta G/G$}
  As discussed above, a direct measurement of $\Delta G/G$ can be found by
  comparing \ha{} or HI rotation curves with stellar rotation curve, where
  the stellar rotation curve must be measured by, e.g. stellar absorption
  lines which arise very close to the surface of dwarf stars.  These curves
  are difficult to observe: depending on brightness, each galaxy may require
  several hours of observation in a large telescope. However, this test
  has the advantage that it does not depend on any detailed dynamical model
  ...
  a handful of these unscreened galaxies can lead to strong constraints on
...
We are grateful Yang et al. for providing the SDSS group data via private
communication.  We thank Mike Jarvis and Elisabeth Krause for helpful
discussions.
